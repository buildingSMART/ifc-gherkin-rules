...
@gherkin_ifc.step("The .{inst_type:property_set_or_element_quantity}. attribute .Name. must use predefined values according to the '{table}' table")
...
@gherkin_ifc.step(
    "Each associated IfcProperty must be of type according to the property set definitions table '{table}'")
@gherkin_ifc.step(
    "Each associated IfcProperty value must be of data type according to the property set definitions table '{table}'")
def step_impl(context, inst, table, inst_type=None):
    ...
    if '.IfcPropertySet. attribute .Name. must use predefined values according' in context.step.name:
            ...
            for property in properties:
                if property.Name not in accepted_values['property_names']:
                    yield ValidationOutcome(inst=inst, expected=accepted_values['property_names'],
                                            observed=property.Name, severity=OutcomeSeverity.ERROR)
        ...
        if 'Each associated IfcProperty must be of type according to the property set definitions table' in context.step.name:
            ...
            for property in inst.HasProperties:
                ...
                if not property.is_a(accepted_property_type):
                    yield ValidationOutcome(inst=inst, expected=accepted_property_type, observed=property,
                                            severity=OutcomeSeverity.ERROR)
