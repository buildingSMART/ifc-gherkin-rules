...
The possible allowed breakdown can be found in the csv file in the folder 'features/resources/spatial_CompositionTable.csv'
  ...
  Scenario: Agreement on each IfcSpatialElement being aggregated to other IfcSpatialStructureElement as per spatial composition table.

      Given A file with Schema Identifier "IFC4X3_TC1" or "IFC4X3_ADD1" or "IFC4X3"
      And An IfcSpatialElement
      Then Each IfcSpatialElement must be aggregated to other IfcSpatialStructureElement as per spatial_CompositionTable.csv