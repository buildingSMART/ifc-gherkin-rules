...
class IfcEntity:
    ...
    def get_alternative_name(self):
        # In case the entity is renamed, such as in the case of 'IfcBuildingElement' being changed to 'IfcBuiltElement'
        self.alternative_name = next(filter(None, map(self.search, [0, 1])), None)
        return self.alternative_name
    
    def get_entity_instances(self, context, include_subtypes = True):
        try:
            return context.model.by_type(self.entity)
        except:
            try:
                return context.model.by_type(self.get_alternative_name(), include_subtypes)
            except:
                return []
