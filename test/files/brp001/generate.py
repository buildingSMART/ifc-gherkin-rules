...
def main():
    def process(fni, shp):
        ...
        prod = f.createIfcBuildingElementProxy(ifcopenshell.guid.new(), ObjectPlacement=f.createIfcLocalPlacement(RelativePlacement=f.createIfcAxis2Placement3D(f.createIfcCartesianPoint((0., 0., 0.)))), Name="Proxy", Representation=pdf)

        fail_or_pass = "fail" if not isv else "pass"
        model_file_name = f'{fail_or_pass}-brp001-{shp_name}_{fn_name}.ifc'
